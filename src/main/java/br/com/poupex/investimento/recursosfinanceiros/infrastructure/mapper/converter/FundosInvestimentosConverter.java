package br.com.poupex.investimento.recursosfinanceiros.infrastructure.mapper.converter;

import br.com.poupex.investimento.recursosfinanceiros.domain.entity.FundosInvestimentos;
import br.com.poupex.investimento.recursosfinanceiros.domain.model.FundosInvestimentosInputOutput;
import lombok.val;
import org.modelmapper.ModelMapper;
import org.modelmapper.spi.MappingContext;
import org.springframework.stereotype.Component;

@Component
public class FundosInvestimentosConverter {

	private final ModelMapper intern = new ModelMapper();

	public FundosInvestimentosConverter(final ModelMapper mapper) {
		mapper.createTypeMap(FundosInvestimentos.class, FundosInvestimentosInputOutput.class).setConverter(this::converter);
	}

	public FundosInvestimentosInputOutput converter(MappingContext<FundosInvestimentos, FundosInvestimentosInputOutput> context) {
		val input = context.getSource();
		val output = intern.map(input, FundosInvestimentosInputOutput.class);
		output.setMensuracaoDescricao(input.getFormaMensuracao().getLabel());
		return output;
	}

}
